...
func Status(n note.CommitNote, options OutputOptions, projPath ...string) (string, error) {
	...
	if options.ApplicationOff {
		n = n.FilterOutApplications()
	} else if options.TerminalOff {
...
func CommitSummary(projects []ProjectCommits, options OutputOptions) (string, error) {
	notes := options.limitNotes(retrieveNotes(projects, options.TerminalOff, options.ApplicationOff, false, "Mon Jan 02"))
	if len(notes) == 0 {
		return "", nil
	}
...
func Commits(projects []ProjectCommits, options OutputOptions) (string, error) {
	notes := options.limitNotes(retrieveNotes(projects, options.TerminalOff, options.ApplicationOff, true, ""))
	if len(notes) == 0 {
		return "", nil
	}
...
func Timeline(projects []ProjectCommits, options OutputOptions) (string, error) {
	notes := options.limitNotes(retrieveNotes(projects, options.TerminalOff, options.ApplicationOff, false, ""))
	if len(notes) == 0 {
		return "", nil
	}
